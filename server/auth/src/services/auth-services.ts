...
class AuthService {
    ...
    async refresh(refreshToken: string) {
        const existingUser = await this.repository.findUserByRefreshToken(refreshToken)
        ...
        if (!existingUser) {
            try {
                ...
                const hackedUser = await this.repository.findUserByUsername((payload as any).username)
