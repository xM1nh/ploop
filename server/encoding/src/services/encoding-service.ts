...
class EncodingService {
  ...
  async encode(id: string): Promise<{ sprayUrl: string; coverUrl: string }> {
    ...
    const imageInputString = imagePaths
      .map((file) => {
        return `file '${file}'`;
      })
      .join("\n");
    ...
    return new Promise((resolve, reject) => {
      ffmpeg()
        ...
        .on("end", async () => {
          ...
          const coverImageFileContent = await fsPromises.readFile(
            imagePaths[imagePaths.length - 1],
          );

          const sprayUrl = await this.repository.uploadVideo(
            id,
            videoFileContent,
          );
          const coverUrl = await this.repository.uploadCoverImage(
            id,
