...
export default (app: Express, channel: Channel) => {
  ...
  app.put(
    ...
    asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
      ...
      const { caption, viewPermission, drawPermission, limited, deadline } =
        req.body;
      ...
      if (viewPermission)
        spray = await service.updateViewPermission(id, viewPermission);
      if (drawPermission)
        spray = await service.updateDrawPermission(id, drawPermission);
      if (limited)
        spray = await service.updateLimitation(id, limited, deadline);
