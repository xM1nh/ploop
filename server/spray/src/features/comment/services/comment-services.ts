...
class CommentService {
  ...
  async getComments(id: number, limit: number, offset: number) {
    const comments = await this.repository.findCommentsBySprayId(
      id,
      limit,
      offset,
    );
  ...
  async comment(sprayId: number, userId: number, comment: string) {
    const newComment = await this.repository.addComment(
      sprayId,
      userId,
      comment,
    );
  ...
  async deleteAllCommentsByUserId(id: number) {
    ...
    deletedComments.forEach((comment: { spray_id: number }) => {
      if (groupedBySprayId[comment.spray_id])
        groupedBySprayId[comment.spray_id]++;
