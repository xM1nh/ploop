...
class LikeService {
  ...
  async getLikes(id: number, limit: number, offset: number) {
    const likes = await this.repository.findLikesBySprayId(id, limit, offset);
    return likes;
  }

  async getLike(sprayId: number, userId: number) {
    const like = await this.repository.findLikeByUserIdAndSprayId(
      sprayId,
      userId,
    );
    return like;
  ...
  async like(sprayId: number, userId: number) {
    const isLike = await this.repository.findLikeByUserIdAndSprayId(
      sprayId,
      userId,
    );
    if (isLike) return;
  ...
  async unlike(sprayId: number, userId: number) {
    const isLike = await this.repository.findLikeByUserIdAndSprayId(
      sprayId,
      userId,
    );
    ...
    const response = await this.repository.deleteLikeByUserIdAndSprayId(
      sprayId,
      userId,
    );
  ...
  async removeAllLikesByUserId(id: number) {
