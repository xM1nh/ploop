...
class _DoublyLinkedBase {
    ...
    protected insertBetween = (e: any, pred: NodeInterface, succ: NodeInterface): NodeInterface => {
...
class PositionalList extends _DoublyLinkedBase {
    ...
    private validate = (p: any): NodeInterface => {
        ...
        if (!(p instanceof this.Position)) throw TypeError(`${p} must be proper Position type`)
        if (p.container !== this) throw new Error(`${p} does not belong to this container`)
    ...
    //-------------------------Mutators------------------------
    private insertBetweenPosition = (e: any, pred: NodeInterface, succ: NodeInterface): PositionInterface | null => {
        const node = super.insertBetween(e, pred, succ)
        return this.makePosition(node)
    }

    public addFirst = (e: any): PositionInterface | null => {
        return this.insertBetweenPosition(e, this.header, this.header.next as NodeInterface)
    }

    public addLast = (e: any): PositionInterface | null => {
        return this.insertBetweenPosition(e, this.trailer.prev as NodeInterface, this.trailer)
    }

    public addBefore = (p: PositionInterface, e: any): PositionInterface | null => {
        const original = this.validate(p)
        return this.insertBetweenPosition(e, original.prev as NodeInterface, original)
    }

    public addAfter = (p: PositionInterface, e: any): PositionInterface | null => {
        const original = this.validate(p)
        return this.insertBetweenPosition(e, original, original.next as NodeInterface)
    }
