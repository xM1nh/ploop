...
class NotificationService {
  ...
  async getNotificationsOfUser(userId: number, limit: number, offset: number) {
  ...
  async getUnreadNotificationCountOfUser(id: number) {
  ...
  async markAllAsUnread(userId: number) {
  ...
  async subscribeEvents(payload: string) {
    ...
    switch (event) {
      case "CREATE_NOTIFICATION":
        const notification = await this.createNewNotification(
          entityTypeId,
          entityId,
          actorId,
          notifierId,
