...
export const resolvers = {
  ...
  Mutation: {
    addComment: async (
      ...
      const newComment = await dataSources.commentApi.addComment(
        sprayId,
        userId,
        notifierId,
        comment,
    ...
    editComment: async (
      _: any,
      { commentId, newComment }: { commentId: string; newComment: string },
      { dataSources }: { dataSources: DataSource },
    ) => {
    ...
    deleteComment: async (
      _: any,
      { commentId, sprayId }: { commentId: string; sprayId: string },
      { dataSources }: { dataSources: DataSource },
    ) => {
  ...
  Subscription: {
    commentAdded: {
      ...
      subscribe: withFilter(
        (_: any, __: any, { pubsub }: { pubsub: AMQPPubSub }) =>
          pubsub.asyncIterator(["COMMENT_ADDED"]),
        (payload: Comment, variables: { sprayId: string; userId: string }) => {
          return (
            payload.spray_id === parseInt(variables.sprayId) &&
            payload.user_id !== parseInt(variables.userId)
          );
        },
      ),
    },
